...
import { lancamentosMock, categoriasDisponiveis, contasDisponiveis, caixaProjetado } from '../../data/interno'
...
export function InternoCustos() {
  const [lancamentos, setLancamentos] = useState(lancamentosMock)
  ...
  const handleCategoriaChange = useCallback((id, newCat) => {
  ...
  const handleCSVDrop = useCallback((e) => {
    ...
    reader.onload = (ev) => {
      ...
      const parsed = lines.map((line, i) => {
        ...
        return { id: 1000 + i, data: cols[0] || '2026-03-01', descricao: cols[1] || 'Importado', valor: parseFloat(cols[2]?.replace(',', '.') || '0'), categoria: cols[3] || 'Outros', conta: cols[4] || 'Banco Principal' }
  ...
  return (
    <div className="p-8">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-[12px] p-8 mb-8 text-center transition-all ${dragActive ? 'border-[#00C896] bg-[#E6FAF5]' : 'border-[#E2E8F0] bg-white hover:border-slate-300'}`}
      >
            ...
            <tbody className="divide-y divide-[#E2E8F0]">
              {filtrados.map((l) => (
                <tr key={l.id} className="hover:bg-[#F0FDF9] transition-colors">
                  <td className="px-6 py-3 text-sm text-slate-500 whitespace-nowrap">{new Date(l.data + 'T00:00').toLocaleDateString('pt-BR')}</td>
                  ...
                  <td className="px-6 py-3">
                    <select value={l.categoria} onChange={(e) => handleCategoriaChange(l.id, e.target.value)}
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-[12px] shadow-card p-6">
          ...
          {despesasPorCategoria.length > 0 ? (
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={despesasPorCategoria} cx="50%" cy="50%" innerRadius={55} outerRadius={95} paddingAngle={2} dataKey="value">
                  {despesasPorCategoria.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                </Pie>
                <Tooltip content={<PieTooltip />} />
                <Legend wrapperStyle={{ fontSize: '11px', fontFamily: 'Inter' }} formatter={(value) => <span style={{ color: '#64748B' }}>{value}</span>} />
              </PieChart>
            </ResponsiveContainer>
          ...
          {receitasPorTipo.length > 0 ? (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={receitasPorTipo} barSize={32}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fill: '#94A3B8', fontSize: 11, fontFamily: 'Inter' }} tickFormatter={(v) => formatCurrencyShort(v)} width={70} />
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-[12px] shadow-card p-6">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={caixaProjetado}>
            ...
            <YAxis axisLine={false} tickLine={false} tick={{ fill: '#94A3B8', fontSize: 12, fontFamily: 'Inter' }} tickFormatter={(v) => formatCurrencyShort(v)} width={70} />
            <Tooltip content={<ChartTooltip />} />
            <Line type="monotone" dataKey="saldo" stroke="#0D1B2A" strokeWidth={2.5} dot={{ r: 4, fill: '#0D1B2A' }} name="Saldo" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
