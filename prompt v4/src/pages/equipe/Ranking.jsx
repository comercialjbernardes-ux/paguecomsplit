...
function PodioCard({ vendedor, posicao, periodo }) {
  ...
  const medalColors = { 1: 'text-[#00C896]', 2: 'text-slate-400', 3: 'text-amber-500' }
  ...
  return (
    <div className={`flex flex-col items-center ${posicao === 1 ? 'order-2' : posicao === 2 ? 'order-1' : 'order-3'}`}>
      ...
      <div className={`w-32 ${heights[posicao]} border rounded-t-[8px] flex flex-col items-center justify-start pt-3 gap-1 ${podiumColors[posicao]}`}>
        <span className={`text-2xl font-black ${posicao === 1 ? 'text-[#00C896]' : posicao === 2 ? 'text-slate-500' : 'text-amber-500'}`}>
          #{posicao}
        </span>
...
export function EquipeRanking() {
  ...
  const ranked = useMemo(() => {
    const filtered = regiaoFiltro === 'Todas' ? vendedores : vendedores.filter((v) => v.regiao === regiaoFiltro)
      ...
      {top3.length >= 3 && (
        <div className="card p-8 mb-8">
          <div className="flex items-end justify-center gap-4">
            {top3.map((v, i) => (
              <PodioCard key={v.id} vendedor={v} posicao={i + 1} periodo={periodo} />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="card overflow-hidden">
        ...
        <div>
          {ranked.map((v, idx) => {
            ...
            const pct = calcularAtingimento(fat, periodo === 'mensal' ? v.metaMensal : v.metaAcumulada)
            const posicao = idx + 1
            const isTop3 = posicao <= 3
            const isPositivo = v.variacaoMes >= 0

            return (
              <div key={v.id} className="flex items-center gap-4 px-6 py-4 border-b border-[#E2E8F0] last:border-0 hover:bg-[#F0FDF9] transition-colors duration-100">
                <div className="w-10 flex-shrink-0 text-center">
                  <span className={`text-xl font-black ${posicao === 1 ? 'text-[#00C896]' : posicao === 2 ? 'text-slate-400' : posicao === 3 ? 'text-amber-500' : 'text-slate-300'}`}>
                    {posicao}
                  </span>
                </div>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isTop3 ? 'bg-[#0D1B2A]' : 'bg-slate-100'}`}>
                  <span className={`text-xs font-bold ${isTop3 ? 'text-white' : 'text-slate-500'}`}>{v.avatar}</span>
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center justify-end gap-0.5 mt-0.5">
                    ...
                    <span className={`text-xs font-medium ${isPositivo ? 'text-[#10B981]' : 'text-[#EF4444]'}`}>
                ...
                <div className="w-24 text-right">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                    pct >= 100 ? 'bg-[#ECFDF5] text-[#065F46]' : pct >= 70 ? 'bg-[#FFFBEB] text-[#92400E]' : 'bg-[#FEF2F2] text-[#991B1B]'
                  }`}>
                    {pct}%
                  </span>
                </div>
