...
export function InternoTransacoes() {
  ...
  const { refetch, currentSheetId } = useSheetsData()
  ...
  const handleSave = async (data: LancamentoFormData) => {
    ...
    try {
      if (editItem) {
        await updateLancamento(editItem, data, currentSheetId)
        showFeedback('success', `Lancamento "${data.descricao}" atualizado com sucesso`)
      } else {
        await addLancamento(data, currentSheetId)
        showFeedback('success', `Lancamento "${data.descricao}" criado com sucesso`)
      }
      setIsFormOpen(false)
      setEditItem(null)
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteLancamento(deleteItem, currentSheetId)
              ...
              {paginados.length === 0 ? (
                ...
                paginados.map((l) => (
                  <tr key={l.id} className="border-b border-gray-50 hover:bg-gray-50 group">
                    ...
                    <td className="py-3 px-3 font-medium text-gray-900 max-w-[250px] truncate">{l.descricao}</td>
