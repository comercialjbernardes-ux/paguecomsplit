...
export function InternoCarteira() {
  ...
  const [vendedorFiltro, setVendedorFiltro] = useState(0) // 0 = todos
  ...
  const segmentosData = useMemo(() => {
    ...
    const source = vendedorFiltro !== 0
      ? clientesCarteira.filter((c) => c.vendedorId === vendedorFiltro)
      : clientesCarteira
    source.forEach((c) => {
    ...
    <div className="p-8">
      ...
      <div className="flex items-center gap-4 mb-6 flex-wrap">
        ...
        <select
          ...
          onChange={(e) => setVendedorFiltro(Number(e.target.value))}
          className="h-9 px-3 text-sm border border-[#E2E8F0] rounded-[8px] bg-white text-[#0D1B2A] font-medium focus:outline-none focus:ring-2 focus:ring-[#00C896]/30"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        {segmentosData.map((s) => (
          <div
            key={s.segmento}
            onClick={() => setSegmentoFiltro(segmentoFiltro === s.segmento ? 'Todos' : s.segmento)}
              ...
              {filtrados.map((c) => (
                <tr key={c.id} className="hover:bg-[#F0FDF9] transition-colors">
                  ...
                  <td className="px-6 py-3.5">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#F8FAFC] text-slate-600 border border-[#E2E8F0]">
                      {c.segmento}
                    </span>
                  </td>
              ...
              return (
                <div key={c.id} className="flex items-center gap-4 px-6 py-3 hover:bg-[#F0FDF9] transition-colors">
                  <span className={`text-lg font-black w-7 text-center ${
                    posicao <= 3 ? 'text-[#00C896]' : 'text-slate-300'
                  }`}>
                    {posicao}
                  </span>
