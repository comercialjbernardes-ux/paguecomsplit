...
type StatusCliente = 'ativo' | 'em risco' | 'novo'

// ─── Sub-components ────────────────────────────────────────────────────────

function StatusPgtoBadge({ status }: { status: StatusPgto }) {
  const map = {
...
function StatusClienteBadge({ status }: { status: StatusCliente }) {
  const map: Record<StatusCliente, { label: string; cls: string }> = {
...
// Select estilizado com placeholder
interface SelectVendedorProps {
  value: string
  onChange: (v: string) => void
}

function SelectVendedor({ value, onChange }: SelectVendedorProps) {
    ...
    <div className="relative w-full">
      <button
        ...
        className={`w-full flex items-center gap-3 h-14 px-5 text-[15px] border-2 rounded-[12px] bg-white transition-all duration-150 ${
          open
            ? 'border-[#00C896] shadow-[0_0_0_3px_rgba(0,200,150,0.15)]'
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#E2E8F0] rounded-[12px] shadow-card-hover z-20 overflow-hidden">
            ...
            {vendedores.map((v) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-5 py-3 transition-colors hover:bg-[#F0FDF9] ${
                  String(v.id) === value ? 'bg-[#E6FAF5]' : ''
                }`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                  String(v.id) === value ? 'bg-[#00C896]' : 'bg-[#0D1B2A]'
                }`}>
                  <span className="text-white text-xs font-bold">{v.avatar}</span>
                </div>
                <div className="text-left flex-1">
                  <p className={`text-sm font-medium ${String(v.id) === value ? 'text-[#00C896]' : 'text-[#0D1B2A]'}`}>
                    {v.nome}
                  </p>
                  <p className="text-xs text-slate-400">{v.regiao}</p>
                </div>
                {String(v.id) === value && (
                  <CheckCircle size={16} strokeWidth={1.5} className="text-[#00C896] flex-shrink-0" />
                )}
              </button>
            ))}
          </div>
...
export function EquipeComissionamento() {
  ...
  const vendedor = useMemo(
    () => vendedores.find((v) => String(v.id) === vendedorId) ?? null,
    [vendedorId],
  )

  const carteira = useMemo(
    () => (vendedor ? carteiraPorVendedor[vendedor.id] : null),
    [vendedor],
  )

  const comissao = useMemo(
    () => (vendedor ? calcularComissaoVendedor(vendedor, periodo) : null),
    [vendedor, periodo],
  )

  const totalComissaoProdutos = useMemo(
    () => (carteira ? calcularComissaoTotalProdutos(carteira.produtos) : 0),
    [carteira],
  )

  const totalFaturamentoProdutos = useMemo(
    () => (carteira ? calcularFaturamentoProdutos(carteira.produtos) : 0),
    [carteira],
  )
      ...
      {!vendedor && (
        <div className="card py-16 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-slate-400 text-sm">
            Selecione um vendedor acima para ver o detalhamento de comissões e carteira.
          </p>
        </div>
      )}
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${
                    comissao.atingimentoPct >= 100
                      ? 'bg-[#ECFDF5] text-[#065F46]'
                      : comissao.atingimentoPct >= 70
            ...
            <div className="card p-6">
              ...
              <div className="flex items-center gap-3 text-xs text-slate-400">
                <span className="text-[#10B981] font-medium">
                  {carteira.clientes.filter((c) => c.status === 'ativo').length} ativos
                </span>
                <span>·</span>
                <span className="text-[#EF4444] font-medium">
                  {carteira.clientes.filter((c) => c.status === 'em risco').length} em risco
                </span>
                <span>·</span>
                <span className="text-blue-500 font-medium">
                  {carteira.clientes.filter((c) => c.status === 'novo').length} novos
                </span>
              </div>
            </div>
          </div>
              ...
              <table className="w-full">
                ...
                <tfoot>
                  <tr className="bg-[#F8FAFC] border-t-2 border-[#E2E8F0]">
                    ...
                    <td className="px-6 py-4" />
                    <td className="px-6 py-4">
                      {/* Total em destaque — Inter 18px / 700 / #00C896 */}
                      <span
          ...
          <div className="card overflow-hidden">
            <div className="px-6 py-4 border-b border-[#E2E8F0] flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  {carteira.clientes.length} clientes ativos em {vendedor.regiao}
                </p>
              </div>
            </div>
            ...
            <div className="divide-y divide-[#E2E8F0]">
              {carteira.clientes.map((c, i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 px-6 py-4 hover:bg-[#F0FDF9] transition-colors duration-100"
                >
                  {/* Ícone */}
