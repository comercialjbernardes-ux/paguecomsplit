...
def _aplicar_overrides(registros: list[dict], overrides: dict[str, dict]) -> None:
    ...
    for r in registros:
        ...
        for campo, valor in ov.items():
            ...
            # Edição manual de URL de afiliados força status encontrado_manual
...
def api_dados():
    ...
    # Re-mescla saúde das URLs em cada request — o worker atualiza continuamente
    _aplicar_url_health(_dados)
    return jsonify(_dados)
...
def api_stats():
    ...
    sem_email = sum(1 for r in _dados if r.get("status") == "nao_encontrado")
    com_afiliados = sum(1 for r in _dados if r.get("url_afiliados"))
    editados = sum(1 for r in _dados if r.get("_editado_manualmente"))
    ultima = max((r.get("data_coleta", "") for r in _dados), default="")

    # Distribuições para filtros
    portes = sorted({r.get("porte_empresa", "") for r in _dados if r.get("porte_empresa")})
    situacoes = sorted({r.get("situacao_cadastral", "") for r in _dados if r.get("situacao_cadastral")})
    ufs = sorted({r.get("uf", "") for r in _dados if r.get("uf")})

    # URL health stats — computa sob demanda sobre _dados atual
    _aplicar_url_health(_dados)
    urls_ativas = sum(1 for r in _dados if r.get("_url_health_status") == "ok")
    urls_redirect = sum(1 for r in _dados if r.get("_url_health_status") == "redirect")
    urls_inativas = sum(1 for r in _dados if r.get("_url_inativa"))
    urls_desconhecido = sum(
        1 for r in _dados if r.get("_url_health_status") == "desconhecido"
    )
